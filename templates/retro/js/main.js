...
function pintarHero(){
  ...
  if (C.fotos.hero) {
    ...
    img.src = C.fotos.hero;
    document.getElementById('hero-photo').prepend(img);
  }
}
...
function pintarMensajes(){
  ...
  document.getElementById('mensajes-contenido').innerHTML = C.mensajes.map(m => `
    <div class="carta reveal">
      <div class="carta-sello"><span>B&amp;S</span></div>
      <p class="carta-texto">${m.texto}</p>
      ${m.referencia ? `<span class="carta-ref">${m.referencia}</span>` : ''}
    </div>`).join('');
}
...
function pintarHistoria(){
  ...
  cont.innerHTML = C.historia.map(h => `
    <div class="polaroid reveal">
      <div class="polaroid-cinta"></div>
      <div class="polaroid-foto"><img src="${h.foto}" alt="${h.titulo || ''}"></div>
      <div class="polaroid-caption">${h.titulo || ''}</div>
    </div>`).join('');
  // agregamos el texto de cada momento debajo, en formato de nota
  const notas = document.createElement('div');
  notas.style.maxWidth = '460px';
  notas.style.margin = '2rem auto 0';
  notas.style.padding = '0 2rem';
  notas.style.textAlign = 'left';
  notas.innerHTML = C.historia.map(h => `
    <p style="font-family:var(--font-mono);font-size:.85rem;color:var(--texto-mid);margin-bottom:.9rem"><b style="color:var(--naranja)">${h.titulo}:</b> ${h.texto}</p>
  `).join('');
  cont.parentElement.appendChild(notas);
}
...
function pintarTimeline(){
  ...
  cont.innerHTML = C.timeline.map((t, i) => `
    <div class="ticket reveal">
      <div class="ticket-num">${String(i + 1).padStart(2,'0')}</div>
      <div class="ticket-info">
        <span class="ticket-titulo">${TuBodaBackend.iconoTimelineHtml(t.icono)}${t.titulo}</span>
        <span class="ticket-hora">${t.hora}</span>
      </div>
    </div>`).join('');
}
...
function iniciarCountdown(){
  ...
  function bloque(v, l){ return `<div class="cd-item"><div class="cd-flip">${String(v).padStart(2,'0')}</div><p>${l}</p></div>`; }
  actualizar();
