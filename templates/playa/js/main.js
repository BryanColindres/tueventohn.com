...
function pintarVestimenta(){
  ...
  if (C.vestimenta.paletaColores && C.vestimenta.paletaColores.length) {
    document.getElementById('vestimenta-paleta').innerHTML = C.vestimenta.paletaColores
      .map(c => `<span class="paleta-color" style="background:${c.hex}" title="${c.nombre || ''}"></span>`).join('');
  }
  if (C.vestimenta.colorEvitar) {
    const p = document.getElementById('vestimenta-evitar');
    p.innerHTML = `Evitemos: <span class="paleta-circulo--inline" style="display:inline-block;width:14px;height:14px;border-radius:50%;vertical-align:middle;margin:0 4px;background:${C.vestimenta.colorEvitar.hex}"></span> ${C.vestimenta.colorEvitar.nombre || ''}`;
...
function iniciarPaginacionFirmas(registros){
  ...
  function render(){
    ...
    if (totalPaginas > 1) {
      ...
      nav.innerHTML = `<button type="button" id="firmas-prev" ${pagina === 0 ? 'disabled' : ''}>‹</button><span>${pagina + 1} / ${totalPaginas}</span><button type="button" id="firmas-next" ${pagina === totalPaginas - 1 ? 'disabled' : ''}>›</button>`;
