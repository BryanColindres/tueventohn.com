...
function iniciarVideo(){
  ...
  document.getElementById('overlay-nombres').textContent = C.festejada.nombre;
...
function pintarHero(){
  document.getElementById('hero-nombre').textContent = C.festejada.nombre;
  document.getElementById('hero-apellidos').textContent = C.festejada.apellido;
...
function aRomano(n){
  const vals = [10,9,5,4,1], sims = ['X','IX','V','IV','I'];
  let r = '';
  for (let i = 0; i < vals.length; i++) while (n >= vals[i]) { r += sims[i]; n -= vals[i]; }
  return r;
}
...
function pintarTimeline(){
  ...
  cont.innerHTML = C.timeline.map((t, i) => `
    <div class="prog-item reveal">
      <span class="prog-num">${aRomano(i + 1)}</span>
...
async function pintarRSVP(){
  ...
  if (!tienePremium) {
    const msj = encodeURIComponent(`Hola, confirmo mi asistencia a los XV años de ${C.festejada.nombre}`);
    document.getElementById('rsvp-festejada').href = `https://wa.me/${C.whatsapp.festejada}?text=${msj}`;
    document.getElementById('rsvp-papas').href = `https://wa.me/${C.whatsapp.papas}?text=${msj}`;
...
function pintarFooter(){
  document.getElementById('footer-iniciales').textContent = C.festejada.nombre;
...
function mostrarPantallaNombre(invitation){
  ...
  document.getElementById('name-screen-sub').textContent = `Mis XV años · ${C.festejada.nombre}`;
...
function pintarMensajeVoz(){
  ...
  document.getElementById('voice-desc').textContent = `${C.festejada.nombre} grabó un mensaje especial para ti`;
